...
import { AKPDQuestion, AKPD_QUESTIONS, getAKPDQuestionsByGrade } from './akpdQuestions';

// Generic function to get questions for any assessment type
// For AKPD, pass gradeLevel (e.g., 'VII', 'VIII', 'IX') to get grade-specific questions
export async function getAssessmentQuestions(type: string, gradeLevel?: string): Promise<Question[] | AKPDQuestion[]> {
  try {
    // Build API URL with optional gradeLevel for AKPD
    let apiUrl = `/api/get-assessment-questions?assessmentId=${type}`;
    if (type === 'akpd' && gradeLevel) {
      apiUrl += `&gradeLevel=${encodeURIComponent(gradeLevel)}`;
    }
  ...
  // Fallback to local grade-specific questions for AKPD
  if (type === 'akpd') {
    if (gradeLevel) {
      return getAKPDQuestionsByGrade(gradeLevel);
    }
    return getAKPDQuestionsWithSupabase();
  }
