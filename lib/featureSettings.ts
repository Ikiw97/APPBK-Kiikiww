...
export interface FeatureSettings {
  ...
  system: {
    ...
    schoolName: string;
    adminEmail: string;
    themeColor: 'blue' | 'purple' | 'green' | 'red' | 'orange' | 'teal' | 'pink' | 'indigo' | 'cyan' | 'slate';
  };
  studentManagement: {
...
export const DEFAULT_SETTINGS: FeatureSettings = {
  ...
  assessments: {
    ...
    riasec: true,      // ✅ RIASEC - Asesmen Minat Karir RIASEC (ENABLED for admin)
    bullying: true,    // ✅ Bullying - Asesmen Pemahaman & Sikap Terhadap Bullying (ENABLED for admin)
    rmib: true,
    sociometry: true,  // ✅ Sociometry (New)
  },
  // Exercise features
  exercises: {
  ...
  // Services (Layanan BK)
  services: {
    counseling_chat: true,     // ✅ Chat Konseling (enabled)
  },
};
...
export async function getFeatureSettings(): Promise<FeatureSettings> {
  ...
  try {
    const response = await fetch(`/api/admin/get-feature-settings?t=${new Date().getTime()}`, {
      headers: {
    ...
    if (success && data) {
      ...
      const mergedSettings: FeatureSettings = {
        assessments: {
          ...
          riasec: data.assessments?.riasec ?? DEFAULT_SETTINGS.assessments.riasec,
          bullying: data.assessments?.bullying ?? DEFAULT_SETTINGS.assessments.bullying,
          rmib: data.assessments?.rmib ?? DEFAULT_SETTINGS.assessments.rmib,
          sociometry: data.assessments?.sociometry ?? DEFAULT_SETTINGS.assessments.sociometry,
        },
        exercises: {
        ...
        system: {
          ...
          themeColor: data.system?.themeColor ?? DEFAULT_SETTINGS.system.themeColor,
        },
        studentManagement: {
        ...
        services: {
          counseling_chat: data.services?.counseling_chat ?? DEFAULT_SETTINGS.services.counseling_chat,
        },
      };
