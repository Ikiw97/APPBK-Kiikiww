...
export default function StudentDashboard({ setCurrentPage, setCurrentSubpage }: StudentDashboardProps) {
  ...
  const handleNavigateToAssessment = () => {
    ...
    if (settings.assessments.riasec) enabledAssessments.push('riasec');
    if (settings.assessments.bullying) enabledAssessments.push('bullying');
    if (settings.assessments.rmib) enabledAssessments.push('rmib');
    if (settings.assessments.sociometry) enabledAssessments.push('sociometry');
      ...
      {assessmentCount > 0 && settings?.assessments && (
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {settings.assessments.sociometry && (
              <FeatureCard
                title="Sosiometri"
                description="Survei hubungan sosial kelas"
                icon={BookOpen}
                available={true}
                onClick={() => handleNavigate('sosiometri')}
              />
            )}
            {settings.assessments.riasec && (
              <FeatureCard
                title="Asesmen Minat Karir RIASEC"
                description="Identifikasi minat karir berdasarkan 6 tipe kepribadian Holland"
                icon={BookOpen}
                available={true}
                onClick={() => handleNavigate('assessment', 'direct-riasec')}
              />
            )}
            {settings.assessments.bullying && (
              <FeatureCard
                title="Asesmen Pemahaman & Sikap Terhadap Bullying"
                description="Evaluasi pemahaman dan sikap siswa terhadap bullying"
                icon={BookOpen}
                available={true}
                onClick={() => handleNavigate('assessment', 'direct-bullying')}
              />
            )}
          </div>
        </div>
  ...
  return (
    ...
    >
      <div className="flex items-start gap-4">
        <div className={`p-2.5 rounded-lg transition-colors ${available ? 'bg-slate-100 text-slate-600 group-hover:bg-primary-50 group-hover:text-primary-600' : 'bg-slate-100 text-slate-400'
          }`}>
          <Icon size={20} />
        </div>
