...
interface GameQuestionEditorProps {
  onBack: () => void;
  fixedGameType?: 'kahoot' | 'vocabulary';
}

export default function GameQuestionEditor({ onBack, fixedGameType }: GameQuestionEditorProps) {
  const { user, loading: authLoading } = useAuth();
  const [gameType, setGameType] = useState<'kahoot' | 'vocabulary'>(fixedGameType || 'kahoot');
  ...
  return (
    <div className="px-6 md:px-8 py-8">
      ...
      <h1 className="text-3xl font-bold text-gray-900 mb-8">
        {fixedGameType === 'kahoot' ? '📝 Editor Soal Kahoot' :
          fixedGameType === 'vocabulary' ? '📝 Editor Vocabulary' :
            '📝 Editor Pertanyaan Game'}
      </h1>
      ...
      {/* Game Type Selector - Only show if no fixed game type */}
      {!fixedGameType && (
        <div className="mb-8">
