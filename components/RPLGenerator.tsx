...
export default function RPLGenerator({ schoolMode }: RPLGeneratorProps) {
    ...
    const [formData, setFormData] = useState({
        ...
        schoolName: 'SMP PUPUK KUJANG', // Satuan Pendidikan
                ...
                {formData[field].map((item, index) => (
                    <div key={index} className="flex gap-2">
                        ...
                        <textarea
                            ...
                            rows={2}
                            className="flex-1 px-4 py-2 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all resize-none text-sm"
                        />
                        <button
        ...
        <div className="flex items-center justify-between mb-6 md:mb-8 px-0 md:px-4">
            {[
                ...
                <React.Fragment key={step.id}>
                    ...
                    >
                        <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center transition-all ${activeStep === step.id ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' :
                            activeStep > step.id ? 'bg-green-500 text-white' : 'bg-slate-100 text-slate-400'
                            }`}>
                            <step.icon size={20} className="md:w-6 md:h-6" />
                        </div>
                        <span className={`text-[10px] md:text-xs font-semibold text-center hidden md:block ${activeStep === step.id ? 'text-blue-600' : 'text-slate-500'}`}>
                            {step.label}
                        </span>
                    </div>
                    {index < arr.length - 1 && (
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div>
                            ...
                            <div className="inline-flex items-center gap-2 bg-blue-50 border border-blue-100 rounded-full px-4 py-1.5 mb-5">
                                ...
                                <span className="text-xs font-bold text-blue-700 uppercase tracking-widest">Perangkat Layanan</span>
                            </div>
                            ...
                            {!showHistory && (
                                <>
                                    <button
                                        ...
                                        className="flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-50 text-blue-700 border border-blue-100 rounded-xl hover:bg-blue-100 transition-all font-medium text-sm"
                                    >
                                        <Download size={18} />
