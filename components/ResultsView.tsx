...
import HollandRIASECResultsList from './HollandRIASECResultsList';
import SMACareerPathResultsList from './SMACareerPathResultsList';
...
const ASSESSMENT_COLORS: Record<string, { gradient: string; bgLight: string; text: string }> = {
  ...
  riasec: { gradient: 'from-orange-500 to-red-600', bgLight: 'bg-orange-50', text: 'text-orange-600' },
  sma_smk: { gradient: 'from-amber-500 to-orange-600', bgLight: 'bg-amber-50', text: 'text-amber-600' },
  bullying: { gradient: 'from-blue-500 to-indigo-600', bgLight: 'bg-blue-50', text: 'text-blue-600' },
...
export default function ResultsView() {
  ...
  const [viewMode, setViewMode] = useState<'all' | 'aum' | 'akpd' | 'emotional_intelligence' | 'gaya_belajar' | 'riasec' | 'bullying' | 'sma_smk'>('all');
  ...
  const loadStats = async () => {
    ...
    try {
      ...
      const allResults = await Promise.all(
        assessmentIds.map(id => getAssessmentResults(id).catch(err => {
          console.warn(`Failed to load results for ${id}:`, err);
          return [];
        }))
      );
  ...
  if (viewMode === 'riasec') {
    return <HollandRIASECResultsList onBack={() => setViewMode('all')} />;
  }

  if (viewMode === 'sma_smk') {
    return <SMACareerPathResultsList onBack={() => setViewMode('all')} />;
  }

  if (viewMode === 'bullying') {
    // For now, use GenericResultsList but we'll need to create BullyingResultsList later
    // This prevents the error by handling bullying separately
    return (
      <GenericResultsList
        assessmentId="bullying"
        onBack={() => setViewMode('all')}
      />
    );
  }
  ...
  if (selectedAssessment) {
    // Prevent bullying from using GenericResultsList directly
    if (selectedAssessment === 'bullying') {
      setViewMode('bullying');
                    ...
                    <div className="flex items-start justify-between mb-5">
                      ...
                      <button
                        onClick={() => {
                          ...
                          } else if (stat.assessment_id === 'riasec') {
                            setViewMode('riasec');
                          } else if (stat.assessment_id === 'sma_smk') {
                            setViewMode('sma_smk');
                          } else {
