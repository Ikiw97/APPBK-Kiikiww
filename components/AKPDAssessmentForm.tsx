...
export default function AKPDAssessmentForm({ onBack, schoolMode }: AKPDAssessmentFormProps) {
  ...
  // Helper function to extract grade level from class (e.g., "VII-1" -> "VII")
  const extractGradeLevel = (kelas: string): string => {
    if (!kelas) return '';
    return kelas.split('-')[0].toUpperCase();
  };

  // Load questions from Supabase (DB) based on grade level
  useEffect(() => {
    const loadQuestions = async () => {
      setQuestionsLoading(true);
      try {
        const gradeLevel = extractGradeLevel(formData.kelas);
        // If kelas is selected, load grade-specific questions
        const data = await getAssessmentQuestions('akpd', gradeLevel || undefined);
        setQuestions(data as AKPDQuestion[]);
      } catch (error) {
        console.error("Error loading AKPD questions:", error);
        // Fallback to local storage/static if DB fails
    ...
    loadQuestions();
  }, [formData.kelas]); // Reload questions when class changes
