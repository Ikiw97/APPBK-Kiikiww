...
import ManajemenGuru from '@/components/ManajemenGuru';
...
import SMACareerPathForm from '@/components/SMACareerPathForm';
import AlatBantuView from '@/components/AlatBantuView';
...
export default function Home({
  ...
  const [activeCaseCount, setActiveCaseCount] = useState<number>(0);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const activeCaseRes = await fetch('/api/admin/get-active-case-count');
        const activeCaseData = await activeCaseRes.json();
        if (activeCaseData.success && activeCaseData.count !== undefined) {
          setActiveCaseCount(activeCaseData.count);
        }
      ...
      <div className="px-6 md:px-10 py-10 bg-[#FAFAFA] min-h-screen font-sans">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
          {[
            {
              label: 'Total Siswa',
              value: registeredStudentCount || '0',
              sub: 'Terdaftar',
              icon: Users,
              bgClass: 'bg-blue-600',
            ...
            <div key={idx} className={`relative overflow-hidden p-6 rounded-3xl border transition-all duration-300 hover:shadow-xl hover:-translate-y-1 group ${stat.bgClass} ${stat.borderClass}`}>
              {/* Decorative Background Icon */}
              <div className={`absolute -right-6 -bottom-6 opacity-10 transform rotate-12 group-hover:scale-110 transition-transform duration-500 text-white`}>
                <stat.icon size={120} />
              </div>

              <div className="relative z-10">
                <div className="flex justify-between items-start mb-6">
                  <div className={`p-3.5 rounded-2xl shadow-sm backdrop-blur-sm ${stat.iconBgClass} ${stat.textClass}`}>
                    <stat.icon size={24} />
                  </div>
                  <span className={`text-xs font-bold px-3 py-1 rounded-full backdrop-blur-sm ${stat.badgeClass}`}>
                    {stat.sub}
                  </span>
                </div>

                <h3 className="text-4xl font-black text-white tracking-tight mb-1">{stat.value}</h3>
                <p className="text-white/80 font-medium text-sm border-l-4 rounded-sm border-white/40 pl-2 ml-1">{stat.label}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="space-y-4">
              ...
              {[
                { title: 'RPL Generator', page: 'rpl', icon: FileText, color: 'text-primary-600' },
                ...
                <div key={i} onClick={() => handleSetCurrentPage(item.page)} className="group flex items-center gap-3 p-3 rounded-xl bg-white border border-slate-100 hover:border-primary-200 hover:shadow-sm cursor-pointer transition-all">
    ...
    <div className="px-6 md:px-8 py-8">
      {user?.role === 'student' && (
        <button
          ...
          className="text-primary-600 hover:text-primary-700 font-medium text-sm mb-6 flex items-center gap-1"
        >
          ← Kembali
        </button>
      )}
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                className={`w-full py-4 px-6 rounded-xl font-semibold transition-all duration-200 flex items-center justify-between group/btn ${schoolMode === 'smp'
                  ...
                  : 'bg-white text-gray-700 border-2 border-gray-100 hover:border-primary-200 hover:bg-primary-50/30'
                  }`}
              >
  ...
  if (showStudentPreview) {
    return (
      <div>
        <div className="sticky top-0 bg-gradient-to-r from-primary-600 to-primary-800 text-white px-6 md:px-8 py-4 shadow-lg z-40">
  ...
  if (currentPage === 'dashboard') {
    ...
    // Handle data-master subpages
    if (currentSubpage === 'data-guru') {
      return <ManajemenGuru />;
    }
    // Default to data-siswa
  ...
  } else if (currentPage === 'counseling-chat') {
    return <CounselingChat onBack={() => handleSetCurrentPage('dashboard')} />;
  ...
  } else if (currentPage === 'alat-bantu') {
    return <AlatBantuView schoolMode={schoolMode} />;
  }
